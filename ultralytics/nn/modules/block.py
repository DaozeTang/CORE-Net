...
class DSPConv(nn.Module):  
    ...
    def __init__(self, c1, c2, k, s):
        ...
        p = [(k, 0, 1, 0), (0, k, 0, 1), (0, 1, k, 0), (1, 0, 0, k)]
        self.pad = [nn.ZeroPad2d(padding=(p[g])) for g in range(4)]
        self.cw = DSConv2D(c1, c2 // 4, (1, k), s=s, p=0)
        self.ch = DSConv2D(c1, c2 // 4, (k, 1), s=s, p=0)
        self.cat = DSConv2D(c2, c2, 2, s=1, p=0)
...
class DSCSPPF(nn.Module):

    def __init__(self, c1, c2, k=5):
        super().__init__()
        c_ = c1 // 2  # hidden channels
        self.cv1 = DSConv2D(c1, c_, 1, 1)
        self.cv2 = DSConv2D(c_ * 4, c2, 1, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
...
class DSCPSABlock(nn.Module):

    def __init__(self, c, attn_ratio=0.5, num_heads=4, shortcut=True) -> None:
        super().__init__()

        self.attn = Attention(c, attn_ratio=attn_ratio, num_heads=num_heads)
        self.ffn = nn.Sequential(DSConv2D(c, c * 2, 1), DSConv2D(c * 2, c, 1, act=False))
        self.add = shortcut
...
class DSC2PSA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, e=0.5):
        ...
        self.cv2 = DSConv2D(2 * self.c, c1, 1)

        self.m = nn.Sequential(*(DSCPSABlock(self.c, attn_ratio=0.5, num_heads=self.c // 64) for _ in range(n)))
...
class DSC2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(DSCBottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class DSC3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        self.m = nn.Sequential(*(DSCBottleneck(c_, c_, shortcut, g, k=((1, 1), (3, 3)), e=1.0) for _ in range(n)))
...
class DSC3k(DSC3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(DSCBottleneck(c_, c_, shortcut, g, k=(k, k), e=1.0) for _ in range(n)))
...
class DSC3k2(DSC2f):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            DSC3k(self.c, self.c, 2, shortcut, g) if c3k else DSCBottleneck(self.c, self.c, shortcut, g) for _ in range(n)
        )
...
class CSPP(nn.Module):
    ...
    def __init__(self, c1, c2, k=5):
        ...
        self.cm = nn.Sequential(Conv(c_, c_ * 2, 1, 1), DWConv(c_ * 2, c_ * 2, k, 1), Conv(c_ * 2, c_, 1, 1))
    ...
    def forward(self, x):
        ...
        return self.cv2(torch.cat((y, y1, y2, y3, y4), 1))


# Shuffle-Pointwise Convolution
class ShufflePointwiseConvolution(nn.Module):
    default_act = nn.SiLU()
    def __init__(self, in_channels, out_channels, k=2, act=True):
        super(ShufflePointwiseConvolution, self).__init__()
        assert in_channels % k == 0
        assert out_channels % k == 0
        self.group_num = k
        self.group_neure = in_channels // k
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), groups=k)
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        x = x.view(b, self.group_num, self.group_neure, h, w).transpose_(2, 1).reshape(b, -1, h, w)
        out = self.conv(x)
...
# Sparsely Factorized Convolution
class SparselyFactorizedConvolution(nn.Module):   
    default_act = nn.SiLU()
    def __init__(self, in_channels, out_channels, k=2, m=2, act=True) -> None:
        super().__init__()
        in_group_neure = k * m
        assert in_channels % in_group_neure == 0
        assert out_channels % m == 0
        mid_neure = in_channels // k
        self.in_group = in_channels // in_group_neure
        self.out_group = m
        self.out_group_neure = out_channels // self.out_group
        
        self.conv1 = nn.Conv2d(in_channels, mid_neure, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), groups=self.in_group)
        self.conv2 = nn.Conv2d(mid_neure, out_channels, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), groups=self.out_group)
    ...
    def forward(self, x):
        n, _, h, w = x.shape
        mid = self.conv1(x)        
        mid = mid.view(n, self.in_group, self.out_group, h, w).transpose_(2, 1).reshape(n, -1, h, w)
        out = self.conv2(mid)
        out = out.view(n, self.out_group, self.out_group_neure, h, w).transpose_(2, 1).reshape(n, -1, h, w)
        return self.act(self.bn(out))
